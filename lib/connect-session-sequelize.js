...
module.exports = function SequelizeSessionInit(Store) {
  ...
  SequelizeStore.prototype.set = function setSession(sid, data, fn) {
    ...
    // If session is found, it means that passport is trying to log the user out so we delete it.
    // Otherwise, create the session.
    this.sessionModel.find({where: {'sid': sid}}).then(function(session) {
      if(!session) {
        debug('Did not find session %s', sid);
      } else {
        return session.destroy().asCallback(fn);
      }
    });

    // If session is not authenticated, don't save it
    if (Object.keys(data.passport).length === 0) {
      returnNull = function (){
    ...
    return this.sessionModel.create({'sid': sid, 'data': stringData, 'expires': expires}).then(function sessionCreated(session) {
